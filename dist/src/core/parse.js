export function parseODataQuery(query) {
    ...
    if (sel)
        opts.select = sel.split(",").map((s) => s.trim()).filter(Boolean);
    ...
    const top = query["$top"];
    if (top !== undefined)
        opts.top = Math.max(0, Number(top));
    const skip = query["$skip"];
    if (skip !== undefined)
        opts.skip = Math.max(0, Number(skip));
    const count = query["$count"];
    if (count !== undefined)
        opts.count = String(count).toLowerCase() === "true";
