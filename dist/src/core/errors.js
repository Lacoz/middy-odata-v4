...
export function validateSelectParameters(select, entityType, edmModel) {
...
export function validateFilterExpression(filter, entityType, edmModel) {
    if (!filter)
        return;
...
export function validateOrderByProperties(orderby, entityType, edmModel) {
...
export function validateExpandNavigationProperties(expand, entityType, edmModel) {
...
export function validateEdmModelConstraints(entity, entityType, edmModel) {
    const entityTypeDef = edmModel.entityTypes?.find(et => et.name === entityType);
    if (!entityTypeDef) {
        throw new ODataBadRequest(`Entity type '${entityType}' not found`);
    }
