...
const DEFAULT_PARSE_OPTIONS = {
    model: {}, // Will be provided by user
    serviceRoot: "",
 ...
 */
export function odataParse(options = {}) {
 ...
 */
function validateQueryOptions(context) {
 ...
 */
function isValidPropertyPath(propertyPath, model) {
    ...
    return model.entityTypes.some((entityType) => entityType.properties?.some((prop) => prop.name === propertyPath));
}
/**
 ...
 * @returns True if valid
 */
function isValidNavigationProperty(navigationPath, model) {
    // Simple validation - in a real implementation, this would be more comprehensive
    if (!model.entityTypes || model.entityTypes.length === 0) {
        return true; // Can't validate without entity types
    }
    // Check if navigation property exists in any entity type
    return model.entityTypes.some((entityType) => entityType.navigation?.some((nav) => nav.name === navigationPath));
}
